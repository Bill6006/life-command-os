...
export function proposeWeeklyDirection(
  ...
  if (allInsufficient) {
    return {
      ...
      minimumWin: 'Record one thing, on one day. That is the whole week.',
      protect: 'Sleep, and whatever is already working',
  ...
  if (capacity === 'depleted' || capacity === 'low') {
    return {
      ...
      kind: 'deliberately-quiet',
      proposal: 'A deliberately quiet week',
      minimumWin: 'Get through it. Nothing is owed.',
      protect: 'Recovery, before anything else',
  ...
  if (focus === undefined) {
    return {
      ...
      kind: 'deliberately-quiet',
      proposal: 'A deliberately quiet week',
      minimumWin: 'Keep doing what is already working',
      protect: 'The routines that are holding',
  ...
  return {
    ...
    minimumWin: 'One block, once. A single protected block is the week done.',
    protect: 'Sleep, and the time already committed to other people',
    basedOn: [...basedOn, `${focus.category} is declining with the most evidence behind it`],
    confidence,
    lastWeek: 'No previous weekly direction recorded',
    /* One wording everywhere. `Adjust` and `Set a direction instead` were the same control. */
    responses: ['Confirm', 'Set a direction instead', 'Snooze', 'Skip'],
  };
}
